          ...
          <VStackFull className="lg:w-90% xl:w-80%" align="items-start">
            ...
            <SVGSection>
              ...
              <SVGText>
                ...
                describe virtually any shape using a series of commands. A
                path's d attribute contains these commands, starting with move
                commands (M or m), line commands (L, l, H, h, V, v), curve
                commands (C, c, S, s, Q, q, T, t), arc commands (A, a), and
...
`}</CodeExample>
              ...
              <SVGText>
                - <HL>H 190</HL>: Draw a horizontal line to x=190 from the
                current y-position.
              </SVGText>
              <SVGText>
              ...
              <SVGText>
                ...
                is a mini language within itself, designed to define the shape
                of the path. It's composed of a series of commands and
                parameters that instruct how the path should be drawn. Let's
                break down the basics and how the numbers work within these
                commands.
              </SVGText>
              <Divider />
              ...
              <SVGText>
                ...
                <HL>M x,y</HL> or <HL>m dx,dy</HL>. The uppercase <HL>M</HL>{" "}
                moves to an absolute position, while the lowercase <HL>m</HL>{" "}
              ...
              <SVGText>
                -<HL> H (horizontal lineto)</HL>: Draws a horizontal line from
                the current position. Syntax: <HL>H x</HL> or <HL>h dx</HL>.
                Uppercase means the line will go to an absolute x-coordinate,
                lowercase means it will move a relative distance from the
                current position.
              </SVGText>
              <SVGText>
                - <HL>V (vertical lineto)</HL>: Draws a vertical line from the
                current position. Syntax: <HL>V y</HL> or <HL>v dy</HL>.
                Uppercase means the line will go to an absolute y-coordinate,
                lowercase means a relative move.
              </SVGText>
              <SVGText>
              ...
              <SVGText>
                - <HL>S (smooth curveto)</HL>: Similar to <HL>C</HL>, but
                assumes the first control point is a reflection of the last
                control point from the previous <HL>C</HL> or <HL>S</HL>{" "}
                command. Useful for chaining Bézier curves. Syntax:{" "}
                <HL>S x2,y2 x,y</HL> or <HL>s dx2,dy2 dx,dy</HL>.
              </SVGText>
              <SVGText>
                - <HL>Q (quadratic Bézier curveto)</HL>: Draws a quadratic
                Bézier curve. Syntax: <HL>Q x1,y1 x,y</HL> or{" "}
                <HL>q dx1,dy1 dx,dy</HL>, where <HL>(x1,y1)</HL> is the control
                point.
              </SVGText>
              <SVGText>
                - <HL>T (smooth quadratic Bézier curveto)</HL>: Like <HL>Q</HL>,
                but assumes the control point is a reflection of the last
                control point from the previous <HL>Q</HL> or <HL>T</HL>{" "}
                command.
              </SVGText>
              <SVGText>
                - <HL>A (elliptical Arc)</HL>: Draws an arc of an ellipse.
                Syntax:
              ...
              <SVGText>
                ...
                commands (M, L, C, etc.) and relative commands (m, l, c, etc.)
                is that absolute commands specify a location or distance from
                the origin <HL>(0,0)</HL> of the SVG canvas, while relative
                commands specify a location or distance from the current point.
              </SVGText>
              <SVGText>
                The numbers specified in SVG path commands, as well as other SVG
                elements' attributes like width, height, coordinates (such as{" "}
                <HL>cx</HL>,<HL>cy</HL> for circles, or <HL>x</HL>, <HL>y</HL>{" "}
                for rectangles), are in pixel units by default when no unit is
              ...
              <SVGText>
                ...
                documents or standalone SVG files viewed in a browser, the
                default unit of measurement is pixels. For example, M10 10 L90
                90 moves to (10,10) pixels from the origin (top-left corner) and
                draws a line to (90,90) pixels.
              </SVGText>
              <SVGText>
                <HL>Viewport Units</HL>: SVGs can also use viewport-relative
                units like vw (viewport width) and vh (viewport height), among
                others, which can be useful for responsive designs. However,
                these units are typically used in style sheets or attributes
                like width and height on the <HL>&lt;svg&gt;</HL> element
                itself, rather than within the d attribute of a{" "}
                <HL>&lt;path&gt;</HL>.
              </SVGText>{" "}
              <Divider />
              <SVGHeading>SVG Scalability</SVGHeading>
              <SVGText>
                Despite the use of pixel units for simplicity and clarity, one
                of the strengths of SVG (Scalable Vector Graphics) is that the
...
`}</CodeExample>
              ...
              />
              <SVGText>
                SVG path commands within the d attribute typically use
                coordinate values that do not directly support viewport units
                like <HL>vh</HL> (viewport height) or <HL>vw</HL> (viewport
                width), because these commands are interpreted in the SVG's own
                coordinate system, which defaults to pixels. However, you can
                design an SVG's overall dimensions using <HL>vh</HL> and{" "}
                <HL>vw</HL> to make the SVG container responsive, and then draw
                paths within this responsive container.
              </SVGText>
              <SVGText>
                Although the d attribute itself doesn't use <HL>vh</HL> or{" "}
                <HL>vw</HL>, the effect of using viewport units on the SVG
                element can be demonstrated by scaling the SVG container. Here's
                how you can create a responsive SVG that scales with the
                viewport, with a path inside.
              </SVGText>{" "}
              <SVGText>
                <HL>SVG Container</HL>: The width and height of the SVG are set
                to 40vw and 40vh, respectively. This means the SVG will take up
                40% of the viewport height in both direcitons, making it
              ...
              <SVGText>
                <HL>Path Command</HL>: The path command draws a square within
                this coordinate system. Regardless of the actual size of the SVG
                on the screen (which scales with the viewport because of vw and
                vh), the path coordinates are defined in terms of the{" "}
                <HL>viewBox</HL> dimensions, allowing the path to scale
                proportionally within the SVG container.
              </SVGText>
              <SVGText>
                By using vw and vh on the SVG element and a suitable viewBox,
                you can create scalable vector graphics that adapt to the
                viewport size, with paths that automatically resize to fit the
                container. This approach maintains the scalability and
                responsiveness of SVG graphics on different screen sizes,
                ensuring your graphics look great on any device.
              </SVGText>
              <Divider />
              <SVGHeading>The viewBox attribute</SVGHeading>
              <SVGText>
                The viewBox attribute in an SVG element is a critical feature
                for creating scalable graphics. It defines the aspect ratio and
                the portion of the canvas to be displayed. The viewBox attribute
              ...
              <SVGText>
                ...
                these coordinates. This makes it possible to create graphics
                that scale fluidly with the size of the SVG container, ensuring
              ...
              <SVGText>
                ...
                coordinate system works and using it to your advantage. There
                are two main strategies to consider.
              </SVGText>
              <SVGText>
                If your animation or graphic is not centered in the SVG canvas,
                you can adjust the viewBox and the coordinates of your SVG
                content to center it. The viewBox attribute creates a virtual
                canvas, allowing you to control which part of the SVG content is
                visible and how it's scaled.
              </SVGText>
              <SVGText>
              ...
              <SVGText>
                ...
                content is centered within this virtual canvas. This might
                involve changing the minX and minY values to shift the visible
                area.
              </SVGText>
  ...
  <!-- Your centered content here -->
              ...
              <SVGText>
                ...
                while CSS or SVG transformations (e.g., transform: translate)
                can help center the SVG within its container or elements within
                the SVG. The right approach depends on whether you're adjusting
                the overall SVG or individual elements and animations within it.
              </SVGText>
            </SVGSection>
          </VStackFull>
        </FlexFull>
